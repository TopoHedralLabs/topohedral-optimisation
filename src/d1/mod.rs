...
impl fmt::Display for ScalarError
{
    ...
    ) -> fmt::Result
    {
        match self
        {
            ScalarError::MaxIterReached => write!(f, "Maximum number of iterations reached"),
            ScalarError::BracketNotFound((iter, cond1, cond2, cond3)) =>
            {
                write!(f, "Bracket not found with num iterations: {iter}, condition 1: {cond1}, condition 2: {cond2}, condition 3: {cond3}")
            }
            ScalarError::BadOptions(msg) => write!(f, "Bad options: {}", msg),
            ScalarError::NanEncountered(msg) => write!(f, "NaN encountered: {}", msg),
            ScalarError::InfEncountered(msg) => write!(f, "Inf encountered: {}", msg),
            ScalarError::UnknownError => write!(f, "Unknown error"),
        }
    }
}
//..................................................................................................
...
pub fn bracket<F: Fn(f64) -> f64>(
    f: F,
    opts: &BracketOptions,
) -> Result<(f64, f64, f64, f64, f64, f64, usize), ScalarError>
{
    let g = GOLDERN_RATIO;
    ...
    {
        Err(ScalarError::BracketNotFound {
...
impl<F: Fn(f64) -> f64> Brent<F>
{
    ...
    fn get_bracket_bounds(&self) -> Result<(f64, f64, f64, f64, f64, f64, usize), ScalarError>
    {
        let out = match self.bounds
        {
            ...
            Bounds::Triple((a, b, c)) =>
            {
                let (cond1, cond2, cond3) = brack_conds(a, b, c, &self.f);
                ...
                {
                    Err(ScalarError::BracketNotFound((0, cond1, cond2, cond2)))
                }
            }
        };
        out
    ...
    fn optimize(&mut self) -> Result<(), ScalarError>
    {
        ...
        {
            Err(ScalarError::MaxIterReached)
        }
        else
        {
            Ok(())
...
impl<F: Fn(f64) -> f64> Bounded<F>
{
    ...
    ) -> Result<Self, ScalarError>
    {
        ...
        {
            Err(ScalarError::BadOptions(format!(
                "Invalid bounds {}",
                opts.bounds
            )))
        };
    ...
    ) -> Result<(), ScalarError>
    {
        ...
        {
            Err(ScalarError::NanEncountered(format!(
                "xf = {} fs = {} fu = {}",
                xf, fx, fu
            )))
        }
        ...
        {
            Err(ScalarError::InfEncountered(format!(
                "xf = {} fs = {} fu = {}",
                xf, fx, fu
            )))
        }
