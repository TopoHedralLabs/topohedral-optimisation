...
//}}}
//{{{ dep imports
use topohedral_linalg::{self as la};
use topohedral_tracing::*;
//}}
//{{{ re-exports
pub type SVector<const N: usize> = la::scvector::SCVector<f64, N>;
pub type SMatrix<const N: usize, const M: usize> = la::smatrix::SMatrix<f64, N, M>;
pub use topohedral_linalg::smatrix::EvaluateSMatrix;
pub use topohedral_linalg::GreaterThan;
pub use topohedral_linalg::VectorOps;
pub use topohedral_linalg::*;
//}}}
//--------------------------------------------------------------------------------------------------
...
where
    ...
    [(); N * N]:,
{
    fn eval(&mut self, _x: &SVector<N>) -> f64 {
        0.0
    }
    fn grad(&mut self, _x_in: &SVector<N>) -> SVector<N> {
        SVector::<N>::zeros()
    }
}
//}}}
...
mod tests {
        ...
        ($test_name: ident, $point: expr) => {
            ...
            fn $test_name() {
                ...
                let grad_f_true = |x_vec: &SVector<2>| {
                    let x = x_vec[0];
                    let y = x_vec[1];
                    let out = SVector::<2>::from_col_slice(&[
                ...
                let x = SVector::<2>::from_col_slice(&$point);
        ...
        ($test_name: ident, $point: expr) => {
            ...
            fn $test_name() {
                ...
                let grad_f_true = |x_vec: &SVector<2>| {
                    let x = x_vec[0];
                    let y = x_vec[1];
                    let out = SVector::<2>::from_col_slice(&[x.cos() * y.sin(), x.sin() * y.cos()]);
                    out
                };
    ...
    sin_test!(
        test_sin_3,
        [10000.0 * f64::consts::PI, 10000.0 * f64::consts::PI]
    );
    sin_test!(
        test_sin_4,
        [-10000.0 * f64::consts::PI, -10000.0 * f64::consts::PI]
        ...
        ($test_name: ident, $point: expr) => {
            ...
            fn $test_name() {
                ...
                let grad_f_true = |x_vec: &SVector<2>| {
                    ...
                    let y = x_vec[1];
                    let out =
                        SVector::<2>::from_col_slice(&[(1.0 / x) * y.ln(), x.ln() * (1.0 / y)]);
                    out
                };
